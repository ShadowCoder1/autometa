...
|---|---|---|---|---|---|---|---|
| Claude Fable 5 | `claude-fable-5` | 1M | 128k | $10 / $50 | **always on** (adaptive); `{type:"disabled"}` -> 400 | low, medium, high, xhigh, max | Same tokenizer as Opus 4.7/4.8 (~30% more tokens than pre-4.7 models). Safety classifiers can return `stop_reason:"refusal"`. Requires 30-day data retention ("Covered Model"; not ZDR-eligible — ZDR orgs get `400 invalid_request_error`; verified on live `manage-claude/api-and-data-retention` page 2026-08-15). |
...
* **Not supported (400)**: recursive schemas, complex enum members, external `$ref`, `minimum`/`maximum`/`multipleOf`, `minLength`/`maxLength`, other array constraints, `additionalProperties` other than false. `pattern` **is** supported for simple regexes (`^...$`, `* + ? {n,m}`, `[]`, `.`, `\d\w\s`, groups); backreferences, lookaround, `\b`, and large `{n,m}` ranges are not, and complex patterns may 400 (docs "Pattern support (regex)" accordion). Note the Python SDK's `parse()`/`transform_schema` still strips `pattern` out of the wire schema and appends it to the description, so patterns are enforced client-side only when you go through the SDK helper.
...
* ~~Whether `pattern` in a JSON schema is rejected (400) or silently ignored.~~ Resolved (fact-check): simple regex patterns are supported; complex ones may 400. See §8.
...
* ~~Fable 5 30-day-retention requirement is stated in the skill/migration guide, not re-read on a live page today.~~ Resolved (fact-check): confirmed on the live data-retention page.
* ~~Sonnet 5 support for mid-conversation `role:"system"` messages: live prompt-caching page says yes; the bundled skill says no — trust the live page but test.~~ Resolved (fact-check): the dedicated live page `build-with-claude/mid-conversation-system-messages` lists Claude Sonnet 5 as supported (no beta header); only the *tool-changes* beta excludes Sonnet 5.
...
Independent re-check on 2026-08-15 against primary sources (live `platform.claude.com/docs/en/*.md` pages fetched raw, `anthropics/anthropic-sdk-python` `main`, PyPI JSON). Twelve load-bearing claims were selected; each verdict below names the source used.

| # | Claim (section) | Verdict | Source |
|---|---|---|---|
| 1 | Model pricing: Fable 5 $10/$50, Opus 5 $5/$25, Sonnet 5 **$2/$10 now permanent** (Sept-1 increase to $3/$15 cancelled), Haiku 4.5 $1/$5; cache 1.25x/2x/0.1x; batch 50%; web search $10/1k; code exec 1,550 free h then $0.05/h and free with `web_search_20260209`/`web_fetch_20260209`; fast mode $10/$50 Opus 5/4.8 only (§1) | **confirmed** | https://platform.claude.com/docs/en/about-claude/pricing.md (incl. `claude-sonnet-5-introductory-pricing` note) |
| 2 | Rate limits: Start/Build/Scale RPM-ITPM-OTPM table, spend caps $500/$1,000/$200,000, only `input_tokens`+`cache_creation_input_tokens` count toward ITPM, Opus 5 / Sonnet 5 separate buckets, Batches 1,000/200k/100k etc., token-counting RPM 2,000/4,000/8,000, Files ~100 RPM (§2) | **confirmed** | https://platform.claude.com/docs/en/api/rate-limits.md ; https://platform.claude.com/docs/en/build-with-claude/token-counting.md ; https://platform.claude.com/docs/en/build-with-claude/files.md |
| 3 | PDF: 32 MB request, 600 pages (100 when context < 1M), each page rasterized to image + text extracted, text ≈1,500–3,000 tok/page + image tokens, "Place PDFs before text"; PDF-page bbox coordinates not mappable (rasterized server-side) (§3) | **confirmed** | https://platform.claude.com/docs/en/build-with-claude/pdf-support.md ; https://platform.claude.com/docs/en/build-with-claude/vision-coordinates.md |
| 4 | Vision: `⌈w/28⌉×⌈h/28⌉` visual tokens; high-res tier (Claude 4.7+) 2576 px / 4784 tokens, standard 1568/1568; 600 images/request (100 on 200k models); 10 MB/image; 8000×8000; >20 images -> ≤2000 px rule; 1000×1000 = 1296 tok ≈ $6.48/1k on Opus 5, 4K ≈ $23.92/1k; banker's rounding in resize (§4) | **confirmed** | https://platform.claude.com/docs/en/build-with-claude/vision.md ; https://platform.claude.com/docs/en/build-with-claude/vision-coordinates.md |
| 5 | Citations: `start_page_number` 1-indexed / `end_page_number` exclusive; all-or-none per request; `cited_text` not billed as output (nor input on replay); image/PDF-image citations unsupported; citations + `output_config.format` -> 400 (§5, §8) | **confirmed** | https://platform.claude.com/docs/en/build-with-claude/citations.md ; https://platform.claude.com/docs/en/build-with-claude/structured-outputs.md |
| 6 | Files API: beta header `files-api-2025-04-14`, 500 MB/file, **500 GB/org**, filenames 1–255 chars w/ forbidden chars, persist until deleted, uploads not downloadable (400), free ops, Claude API / Platform on AWS / Foundry only, not ZDR-eligible (§6) | **confirmed** (note: the bundled skill's "100 GB" figure is stale; live page says 500 GB) | https://platform.claude.com/docs/en/build-with-claude/files.md |
| 7 | Batches: 100,000 requests or 256 MB, 24 h expiry, 29-day results, `custom_id` `^[a-zA-Z0-9_-]{1,64}$`, unordered results, unsupported params (`stream`, `speed`, `store`/threads, `cache_hint`/`context_hint`, `max_tokens:0`), `output-300k-2026-03-24` beta for Opus 5/4.8/4.7/4.6 + Sonnet 5/4.6, `fallbacks` -> errored item (§7) | **confirmed** | https://platform.claude.com/docs/en/build-with-claude/batch-processing.md ; https://platform.claude.com/docs/en/build-with-claude/refusals-and-fallback.md |
| 8 | Structured outputs: model list; limits 20 strict tools / 24 optional params / 16 union params / 180 s compile timeout; grammar cache 24 h from last use; `minItems` only 0/1; `additionalProperties:false` required (§8) | **confirmed** | https://platform.claude.com/docs/en/build-with-claude/structured-outputs.md |
| 9 | `pattern` in JSON schema "[UNVERIFIED whether it 400s or is ignored]" (§8, §14) | **corrected** — the docs have a "Pattern support (regex)" accordion: simple patterns are supported (anchors, `* + ? {n,m}`, classes, groups); backreferences, lookaround, `\b`, large `{n,m}` are not; complex ones may 400. The Python SDK's `transform_schema` still strips `pattern` into the description (verified in `src/anthropic/lib/_parse/_transform.py`). Text updated in place. | https://platform.claude.com/docs/en/build-with-claude/structured-outputs.md ; https://github.com/anthropics/anthropic-sdk-python/blob/main/src/anthropic/lib/_parse/_transform.py |
| 10 | Prompt-cache minimums 512 (Opus 5/Fable 5/Mythos 5), 1,024 (Opus 4.8, Sonnet 5, Sonnet 4.6/4.5), 4,096 (Haiku 4.5); 20-block lookback; mid-conversation `role:"system"` on Fable 5/Mythos 5/Opus 5/Opus 4.8/**Sonnet 5** (§9, §14) | **confirmed** — Sonnet 5 support is stated on both the prompt-caching page and the dedicated mid-conversation-system-messages page (no beta header); only the *tool-changes* beta excludes Sonnet 5. §14 item resolved. | https://platform.claude.com/docs/en/build-with-claude/prompt-caching.md ; https://platform.claude.com/docs/en/build-with-claude/mid-conversation-system-messages.md |
| 11 | Fable 5 requires 30-day retention (not ZDR) — was tagged [UNVERIFIED on live page] (§1, §14) | **confirmed** — Fable 5 / Mythos 5 are "Covered Models"; ZDR orgs get `400 invalid_request_error`; per-workspace 30-day override available. Tag removed; §14 item resolved. | https://platform.claude.com/docs/en/manage-claude/api-and-data-retention.md |
| 12 | Server-side fallbacks: `fallbacks:"default"` + `server-side-fallback-2026-07-01`; explicit list up to 3 models also works under `-2026-06-01`; not in Batches/Bedrock/Vertex/Foundry; sticky ~1 h org-scoped; refusal categories incl. `general_harms` (§10) | **confirmed** (nuance: the `-2026-07-01` header accepts *both* forms; `-2026-06-01` accepts the list form only) | https://platform.claude.com/docs/en/build-with-claude/refusals-and-fallback.md |
| 13 | Python SDK 0.122.0 on PyPI, `>=3.9`; default timeout 600 s; non-streaming `ValueError` when `3600*max_tokens/128000 > 600` (i.e. `max_tokens` > 21,333) (§12) | **confirmed** | https://pypi.org/pypi/anthropic/json ; https://github.com/anthropics/anthropic-sdk-python/blob/main/src/anthropic/_base_client.py (`_calculate_nonstreaming_timeout`) ; `_constants.py` |
| 14 | Code execution: Python 3.11, 1 CPU / 5 GiB RAM / 5 GiB disk, no internet, 30-day container expiry (checkpoint after ~5 min idle), 90 s per-cell limit in `_20260521`, `_20260209` web tools need `_20260120`+, library list w/o OpenCV/scikit-image/tesseract, upload types list (§11) | **confirmed** (still unverified: PDF as `container_upload` — the docs list is "including", non-exhaustive) | https://platform.claude.com/docs/en/agents-and-tools/tool-use/code-execution-tool.md |
| 15 | Effort supported models exclude Haiku 4.5; `xhigh` on Fable 5/Mythos 5/Opus 5/Opus 4.8/4.7/Sonnet 5; default `high`; knowledge cutoffs Fable Jan 2026 / Opus 5 May 2026 / Sonnet 5 Jan 2026 / Haiku 4.5 Feb 2025 (§1) | **confirmed** | https://platform.claude.com/docs/en/build-with-claude/effort.md ; https://platform.claude.com/docs/en/about-claude/models/overview.md |

Still **unverifiable** from primary docs (unchanged): Batches accepting `file_id` sources via the non-beta `client.messages.batches` (docs show base64 PDF batch example only); PDF via `container_upload`; whether Claude can visually inspect images generated inside the code-execution container within the same turn.
