...
1. **The tables first.** Work through every table on these pages cell by cell. Find the row that
   carries this outcome and the columns that carry these two groups, and read the two cells. Copy
   the row label and the column label with the value.
2. **Then the running text**, for a sentence that prints the value for each group.
